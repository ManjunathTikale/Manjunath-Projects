...
router.post("/", verifyJWT,  upload.single('image'), async (req, res) => {
   const { title, description, rating, trailer, year, extra, genre, writers, directors  } = req.body
   console.log(req.file)

//    if(!title || !description || !req.file.path || !rating || !trailer || !year) {
    if(!title || !description || !rating || !trailer || !year || !extra) {
...
//    const writersArray = writers ? writers.split(",") : []
   ...
   try {
       ...
       res.status(201).json({
        ...
        res.status(500).json({
            success: false,
            message: "Internal 4 ok Server Error."
        })
   }
})
...
router.patch("/update-details", verifyJWT,  upload.single('image'), async (req, res) => {
    const { id, title, description, rating, trailer, year, extra, genre, writers, directors } = req.body
 
    if(!id || !title || !description || !rating || !trailer || !year || !extra) {
     return res.status(400).json({
         success: false,
         message: "Please fill all required fields."
     })
